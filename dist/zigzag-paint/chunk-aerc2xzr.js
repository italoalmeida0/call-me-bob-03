...
def _bob_find_banned(user_src, banned_names, banned_attrs):
    ...
    for node in _bob_ast.walk(tree):
        if isinstance(node, _bob_ast.Call):
            f = node.func
            if isinstance(f, _bob_ast.Name) and f.id in banned_names:
                return f.id + "()"
            if isinstance(f, _bob_ast.Attribute) and f.attr in banned_attrs:
                return "." + f.attr + "()"
    return None

def bob_grade(user_src, func_name, tests_literal, banned_literal):
...
async function grade(code2, exercise) {
  ...
  const out = await pyodide.runPythonAsync("bob_grade(__bob_src, __bob_func, __bob_tests, __bob_banned)");
...
var _tmpl$25 = /* @__PURE__ */ template(`<div class="h-full flex flex-col overflow-hidden"><div class=status-bar><span></span><span class=text-[#57534e]></span><span class=flex-1></span><span class=text-[#44403c]>call-me-bob 03`);
